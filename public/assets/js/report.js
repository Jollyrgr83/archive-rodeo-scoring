$(() => {
  ...
  $(document).on("click", ".button", event => {
    $.get("/api/retrieve-report/", data => { window.location.href = "/api/retrieve-report/"; });
  });
  ...
  function renderTierMenu() {
    ...
    for (let i = 0; i < event_id_array.length; i++) {
      ...
      optionArray.push({
        id: `${tier_id}-${event_id}`,
        text: `${obj.tier_ref[tier_id].name} - ${obj.event_ref[event_id]}`,
      });
    }
  ...
  function renderScores() {
    const content = [ { text: "", fontSize: 22 }, { table: { headerRows: 1, widths: [ "15%", "40%", "15%", "15%", "15%" ], body: [ [ "Competitor Number", "Organization", "Place", "Score", "Time" ] ] } } ];
    $("#dynamic").empty();
    ...
    for (let i = 0; i < x; i++) {
      ...
      if (org_id === "all" || org_id === obj.comp_ref[competitor_id].org_id) {
        ...
        pTimeEl.text(disp(obj.tiers[tier_id][event_id][i].total_seconds));
  ...
  function disp(tot) {
    var tmin = Math.floor(tot / 60).toFixed(0);
    var tsec = Math.floor(tot % 60).toFixed(0);
    var trem = ((tot % 60) - tsec).toFixed(2);
    var tminDisp = tmin.length === 2 ? tmin : `0${tmin}`;
    var tsecDisp = tsec.length === 2 ? tsec : `0${tsec}`;
    var tremDisp = `${trem[2]}${trem[3]}`;
    return `${tminDisp}:${tsecDisp}.${tremDisp}`;
  }
});
