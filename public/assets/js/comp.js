$(() => {
  ...
  const renderCompSelectionMenu = data => {
    ...
    for (let i = 0; i < data.length; i++) {
      const optionEl = h({ e: "option", v: data[i].id, dt: data[i].team, tx: data[i].text });
  ...
  const renderIndividual = data => {
    ...
    const compNumTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Competitor Number" });
    const compNumEl = h({ e: "input", i: "comp-number", v: data.comp_number });
    const firstTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "First Name" });
    ...
    const saveEl = h({ e: "button", i: "comp-save-button", di: data.id, c: "button", tx: "Save Changes" });
  ...
  const renderTeam = data => {
    ...
    const groupTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Group Names" });
  ...
  const renderCompAddInfo = () => {
    ...
    const addCompNumTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Enter Competitor Number" });
    const addCompNumEl = h({ e: "input", i: "add-comp-number", ty: "text" });
    const addFirstTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Enter Competitor's First Name" });
    const addFirstEl = h({ e: "input", i: "add-first-name", ty: "text" });
    const addLastTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Enter Competitor's Last Name" });
    const addLastEl = h({ e: "input", i: "add-last-name", ty: "text" });
    const addTeamTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Enter Competitor Team Name" });
    const addTeamEl = h({ e: "input", i: "add-team-name", ty: "text" });
    const addGroupTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Enter the Group's Names" });
    const addGroupEl = h({ e: "input", i: "add-group-names", ty: "text" });
    const addOrgMenuTitleEl = h({ e: "p", c: "item-title mx-auto", tx: "Select the Competitor's Organization" });
    const addSaveEl = h({ e: "button", i: "add-comp-save-button", c: "button mx-auto", tx: "Add Competitor" });
    $.get(`/api/comp/tier/${parseInt($("#tier-select").val())}`, res => {
      if (res[0].team === 0) {
        $.get("/api/comp/org/", data => {
          const addOrgMenuEl = h({ e: "select", i: "org-select" });
          for (let i = 0; i < data.length; i++) {
            const addOptionEl = h({ e: "option", v: data[i].id, tx: data[i].name });
  ...
  const renderAddMessage = () => {
    ...
    $.ajax("/api/comp/", inputObj).then(() => {
      const pEl = h({ e: "p", c: "item-title mx-auto", tx: "Success! Competitor Added!" });
  ...
  $(document).on("click", ".button", event => {
    ...
    if ($(event.target).attr("id") === "comp-save-button") {
      $.ajax("/api/comp/update/", inputObj).then(() => {
        const pEl = h({ e: "p", c: "item-title mx-auto", tx: "Success! Changes Saved!" });
